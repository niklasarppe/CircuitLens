...
experiment actually cares about: raw logits, attention patterns, and the
induction logit score.
...
def compare_logits(cpu_logits, mps_logits):
    diff = (cpu_logits - mps_logits.to("cpu")).abs()
    return {
        ...
        "total_positions": cpu_logits.shape[0] * cpu_logits.shape[1],
    }


def compare_attention(cpu_attn, mps_attn):
    diff = (cpu_attn - mps_attn.to("cpu")).abs()
    return {
        "max_abs_diff": diff.max().item(),
        "mean_abs_diff": diff.mean().item(),
    }
...
def main():
    ...
    token_pool = exp.build_token_pool(model_cpu)
    samples = [
    ...
    with torch.no_grad():
        for sequence in samples:
            ...
            attn_stats.append(compare_attention(attn_cpu, attn_mps))
    ...
    print()
    if worst(logit_stats, "max_abs_diff") < 1e-3 and total_mismatches == 0:
